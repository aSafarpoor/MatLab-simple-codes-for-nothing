...
if ~any(w=='H')
    switch wr
        ...
        case 'e'
            mflh=hz2erb(mflh);       % convert frequency limits into erb-rate
        case 'b'
            mflh=hz2bark(mflh);       % convert frequency limits into bark
        otherwise
...
switch wr
    ...
    case 'e'
        blim=erb2hz(mflh(1)+[0 1 p p+1]*melinc)*n/fs;
    case 'b'
        blim=barkhz(mflh(1)+[0 1 p p+1]*melinc)*n/fs;
    otherwise
...
switch wr
    ...
    case 'e'
        pf=(hz2erb((b1:b4)*fs/n)-mflh(1))/melinc;
    case 'b'
        pf=(hz2bark((b1:b4)*fs/n)-mflh(1))/melinc;
    otherwise
...
function m = frq2mel(f)
% m = freq2mel (f)
% compute mel value from frequency f
m = 2595 * log10(1 + f./700);
end 

function f = mel2frq(m)
% f = mel2freq (m)
